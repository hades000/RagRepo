...
def init_search_services():
    ...
    try:
        ...
        if vector_store_manager.exists():
            ...
            print("Global vector store not found. Run sync first.")
            vector_store = None

        # Load BM25 index
        bm25_loaded = bm25_service.load_index()
        if not bm25_loaded:
            print("BM25 index not found. Run sync first.")
...
async def search(request: Request, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # CPU-bound: offload to thread pool to avoid blocking the event loop
        loop = asyncio.get_event_loop()
        retrieved_docs = await loop.run_in_executor(
            None,
            lambda: hybrid_retriever.retrieve(
                query=query,
                k=20,
                rerank=use_rerank,
                final_k=top_k
...
Answer:"""
        ...
        # CPU-bound LLM call: offload to thread pool
        answer = await loop.run_in_executor(
            None,
            lambda: llm_provider.generate(prompt_text)
        )
...
async def find_similar(request: Request, user: dict = Depends(get_current_user)):
    """
