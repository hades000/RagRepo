...
async def get_stats():
    ...
    try:
        ...
        # Document counts - Use quoted Prisma table names
        # doc_count = await db.fetchval(
        #     'SELECT COUNT(*) FROM "Document" WHERE file_url IS NOT NULL'
        # )
        # ttd_count = await db.fetchval(
        #     'SELECT COUNT(*) FROM "TtdDocument" WHERE file_url IS NOT NULL'
        # )
        # packaging_count = await db.fetchval(
        #     'SELECT COUNT(*) FROM "PackagingTrialDocument" WHERE file_url IS NOT NULL'
        # )
        # total_docs = (doc_count or 0) + (ttd_count or 0) + (packaging_count or 0)
        ...
        return success_response(
            data={
                ...
                #     'main': doc_count or 0,
                #     'ttd': ttd_count or 0,
                #     'packaging': packaging_count or 0,
                #     'total': total_docs
                # },
...
@admin_bp.route('/usage/by-user', methods=['GET'])
@require_auth  # ✅ Just check JWT is valid
...
@admin_bp.route('/usage/recent', methods=['GET'])
@require_auth  # ✅ Just check JWT is valid
...
@admin_bp.route('/costs/summary', methods=['GET'])
@require_auth  # ✅ Just check JWT is valid
...
@admin_bp.route('/documents/summary', methods=['GET'])
@require_auth  # ✅ Just check JWT is valid
